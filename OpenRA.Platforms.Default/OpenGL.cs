...
namespace OpenRA.Platforms.Default
{
	...
	static class OpenGL
	{
		...
		// Contexts that are not created through SDL (e.g. EGL/ANGLE) may
		// replace this resolver before OpenGL.Initialize is called.
		public static Func<string, IntPtr> GetProcAddress = SDL.SDL_GL_GetProcAddress;
		...
		{
			glGetIntegerv(GL_NUM_EXTENSIONS, out var count);
			for (var i = 0U; i < count; i++)
				if (glGetStringi(GL_EXTENSIONS, i) == extension)
					return true;
		...
		public static bool DetectGLFeatures()
		{
			...
			try
			{
				...
				if (ExtensionSupported("GL_KHR_debug"))
					Features |= GLFeatures.DebugMessagesCallback;
			}
			catch (Exception) { }
